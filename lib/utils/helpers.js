...
async function getOneContact(token, id) {
  // https://api.lexoffice.io/v1/contacts/e9066f04-8cc7-4616-93f8-ac9ecc8479c8
  try {
    const options = {
      method: 'GET',
      uri: `https://api.lexoffice.io/v1/contacts/${id}`,
      json: true,
      headers: {
        Authorization: `Bearer ${token}`,
      },
    ...
    if (response.statusCode !== 200) {
      console.log('Could fetch object!');
...
async function upsertContact(token, entry) {
  try {
    ...
    if (entry.metadata.recordUid) {
      const existingEntry = await getOneContact(token, entry.metadata.recordUid);
      if (existingEntry !== false) {
        options.method = 'PUT';
        options.uri = `https://api.lexoffice.io/v1/contacts/${entry.metadata.recordUid}`;
        options.body.version = existingEntry.version;
      } else {
        options.body.version = 0;
      }
    }
