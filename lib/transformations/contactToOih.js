...
module.exports.contactToOih = (msg) => {
  ...
  if (msg.data.phoneNumbers) {
    ...
    for (const key in msg.data.phoneNumbers) {
      for (let i = 0; i < msg.data.phoneNumbers[key].length; i += 1) {
        if (phoneTypes.indexOf(key) > -1) {
          contactData.push({
            type: key,
            value: msg.data.phoneNumbers[key][i],
            // categories: [],
          });
        } else {
          contactData.push({
  ...
  if (msg.data.addresses) {
    ...
    for (const key in msg.data.addresses) {
      for (let i = 0; i < msg.data.addresses[key].length; i += 1) {
        const streetParts = (msg.data.addresses[key][i].street) ? msg.data.addresses[key][i].street.trim().split(' ') : [''];
        ...
        addresses.push({
          ...
          streetNumber,
          unit: '',
          zipcode: `${msg.data.addresses[key][i].zip}`,
          city: `${msg.data.addresses[key][i].city}`,
          district: '',
          region: '',
          country: `${msg.data.addresses[key][i].countryCode}`,
          primaryContact: '',
          description: `${msg.data.addresses[key][i].supplement}`,
          // categories: [key],
        });
      }
    }
  ...
  if (msg.data.roles) {
    if (msg.data.roles.customer && msg.data.roles.customer.number) {
      contactData.push({
        type: 'custom',
    ...
    if (msg.data.roles.vendor && msg.data.roles.vendor.number) {
      contactData.push({
        type: 'custom',
        value: `vendor_${msg.data.roles.vendor.number}`,
      });
  ...
  if (msg.data.company) {
    ...
    expression.data = {
      firstName: `${msg.data.person.firstName}`,
      lastName: `${msg.data.person.lastName}`,
