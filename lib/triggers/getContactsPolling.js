...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  // Authenticate and get the token from lexoffice
    ...
    */
    ...
    const entries = await getContacts(token, snapshot);

    if (entries) console.log(`Found ${entries.length} records.`);
    ...
    if (entries.length > 0) {
      entries.forEach((element) => {
        ...
        const transformedElement = transform(newMsg, cfg, contactToOih);
